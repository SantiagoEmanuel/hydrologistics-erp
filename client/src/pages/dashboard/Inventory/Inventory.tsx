...
// Separado para evitar re-renderizados masivos y manejar estado local de inputs
function ProductCard({ product, onEdit, onDelete, onQuickStock }: any) {
  ...
  return (
    <div className="flex flex-col items-center justify-between gap-4 rounded-xl border border-gray-200 bg-white p-5 shadow-sm md:flex-row">
      ...
      {/* ACCIONES DE STOCK (Solo si trackStock es true) */}
      {product.trackStock && (
...
function ProductFormModal({
  ...
  // Inicializamos el estado incluyendo wholesalePrice
  const [formData, setFormData] = useState<Partial<Product>>({
    name: product?.name || "",
    price: product?.price || 0,
    wholesalePrice: product?.wholesalePrice || 0, // <--- NUEVO CAMPO
        ...
        <form onSubmit={handleSubmit} className="space-y-5 overflow-y-auto p-6">
          ...
          <div>
            ...
            <input
              ...
              disabled={!formData.trackStock && !product} // Si no controla stock, deshabilitar
              onChange={(e) =>
