...
// Definimos la interfaz aquí para asegurar que coincida con la respuesta del backend
interface ShiftReportData {
  id: string; // <--- EL BACKEND DEVUELVE 'id', NO 'shiftId'
  openedAt: string;
  closedAt?: string;
  operatorName: string;
...
export default function CloseShiftModal({ onClose, onSuccess }: Props) {
  ...
  // Función segura para obtener el ID del archivo
  const getSafeFileName = () => {
    if (!reportData || !reportData.id) return "Reporte_Caja.pdf";
    const shortId = reportData.id.slice(-6); // Ahora usamos .id
          ...
          <div className="p-8 text-center">
            ...
            <div className="flex flex-col gap-3">
              <PDFDownloadLink
                ...
                fileName={getSafeFileName()} // Usamos la función segura
